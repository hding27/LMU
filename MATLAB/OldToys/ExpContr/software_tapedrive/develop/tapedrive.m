...
% Last Modified by GUIDE v2.5 15-Mar-2011 15:57:40
...
if(libisloaded('inpout32')==false)
    msgbox('could not load lpt library.. will not work;(');
    return
end
...
function change_enable(handles, mstate)
    ...
    set(handles.edit4,'Enable',mstate);
...
if (state == 1)
    ...
    maxshots=str2num(get(handles.edit4,'String'));
    ascii(tt);
    ...
    while(mstop==0)
        ...
        for(i=1:size(dirlisting,1))
            if(strcmp(dirlisting(i).name,'tapedrive_prepare'))
                ...
                if (counter >= maxshots || counter <= 0)
                    
                    dlmwrite(filename,'tape_drive');
                    mput(tt,filename);
                    set(handles.text8,'String','!INTERLOCK!');
                end
...
tt=ftp(get(handles.edit1,'String'));
if(strcmp(class(tt),'ftp')==0)
    msgbox('Could not establish connection to ftp server to delete Interlock file..');
    return;
end
...
dirlisting=dir(tt);
for(i=1:size(dirlisting,1))
            if(strcmp(dirlisting(i).name,filename))
            delete(tt,filename);
            end
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
set(hObject,'String','<html>Reset Counter<br>and Interlock');
