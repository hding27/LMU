   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_tapedrive2.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* tapedrive2.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* tapedrive2.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* tapedrive2 (application created by MATLAB Compiler)
		* run_tapedrive2.sh (shell script that can be run 
	...
	All Platforms:            
		* tapedrive2.dll (shared library, file extension 
		  varies by platform)
		* tapedrive2.h (library header file)
		* tapedrive2.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* tapedrive2.ctf (Component Technology File archive)	
	* tapedrive2.xml (documentation files)
	* tapedrive2.pdb (program database file - if 
	  DEBUG option selected)
	* tapedrive2.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* tapedrive2.ctf (Component Technology File archive)
	* tapedrive2_1_0.dll (component containing 
	  compiled M-code)
	...
	* tapedrive2.jar (Java archive containing Java 
	  interface to M-code in tapedrive2.ctf)
	* tapedrive2.ctf (Component Technology File archive,
	  platform-dependent on target machine)
	...
	* <add_in_file>.xla (Excel add-ins found in:
	  C:\Documents and Settings\Admin\My Documents\software_tapedrive\develop\tapedrive2\src\distrib directory)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
