   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_tapedrive.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* tapedrive.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* tapedrive.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* tapedrive (application created by MATLAB Compiler)
		* run_tapedrive.sh (shell script that can be run 
	...
	All Platforms:            
		* tapedrive.dll (shared library, file extension 
		  varies by platform)
		* tapedrive.h (library header file)
		* tapedrive.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* tapedrive.ctf (Component Technology File archive)	
	* tapedrive.xml (documentation files)
	* tapedrive.pdb (program database file - if 
	  DEBUG option selected)
	* tapedrive.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* tapedrive.ctf (Component Technology File archive)
	* tapedrive_1_0.dll (component containing 
	  compiled M-code)
	...
	* tapedrive.jar (Java archive containing Java 
	  interface to M-code in tapedrive.ctf)
	* tapedrive.ctf (Component Technology File archive,
	  platform-dependent on target machine)
	...
	* <add_in_file>.xla (Excel add-ins found in:
	  C:\Documents and Settings\Admin\My Documents\software_tapedrive\develop\untitled1\src\distrib directory)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
