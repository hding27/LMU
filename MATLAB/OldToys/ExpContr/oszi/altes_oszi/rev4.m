...
% Last Modified by GUIDE v2.5 24-Oct-2011 18:17:07
...
function retrieve_trace20_beta(handles)
global rcl;
global x;
global data1;
global data2;
...
p_points=findstr('PNTS_PER_SCREEN',s1)+21;
points=str2num(s1(p_points:p_points+19))
ibuff1=findstr('FIXED_VERT_GAIN',s1)+21;
ibuff2=findstr('_',s1(ibuff1:end));
vs1=str2num(s1(ibuff1:ibuff2(1)+ibuff1-2));
fprintf(obj1,'C2:inspect? "wavedesc"');
s2=fscanf(obj1);
ibuff1=findstr('FIXED_VERT_GAIN',s2)+21;
ibuff2=findstr('_',s2(ibuff1:end));
vs2=str2num(s2(ibuff1:ibuff2(1)+ibuff1-2));

fprintf(obj1,'WAVEFORM_SETUP SP,0,NP,%d,FP,0,SN,0',points);

fprintf(obj1, 'C1:INSPECT? "DATA_ARRAY_1",BYTE')
data_raw1 = fscanf(obj1);
data1=cell2mat(textscan(data_raw1(13:end),'%f'))

fprintf(obj1, 'C2:INSPECT? "DATA_ARRAY_2",BYTE')
data_raw2 = fscanf(obj1);
data2=cell2mat(textscan(data_raw2(13:end),'%f'));
toc
...
%hold on;
plot(handles.axes1,x,data1*vs1,x,data2*scale*vs2,'r');
xlim(handles.axes1,[0 x(end)]);
xlabel(handles.axes1,timebase);
ylabel(handles.axes1,'Voltage [V]');
%plot(handles.axes1,x,data2,'Color','red');
%hold off;
plot(handles.axes2,x,data2*vs2,'r');
xlim(handles.axes2,[0 x(end)]);
xlabel(handles.axes2,timebase);
ylabel(handles.axes2,'Voltage [V]');
% ymin=min(data2);
