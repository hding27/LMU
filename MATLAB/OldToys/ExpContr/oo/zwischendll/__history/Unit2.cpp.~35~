...
#pragma argsused
	...
	unsigned long (__cdecl *PHR4000_GetCoefficients_LVD)(char* serial, float* coeffs, long* nlorder);
	HINSTANCE ooLib = NULL;

extern "C" __declspec(dllexport) void __cdecl get_Y(long inttime, float* arr);
extern "C" __declspec(dllexport) void __cdecl get_X(float* arr);
...
__declspec(dllexport) void __cdecl get_Y(long inttime, float* arr){
	...
	inppar[2]=inttime;	//Sample Frequency
...
//---------------------------------------------------------------------------
__declspec(dllexport) void __cdecl get_X(float* arr){
	...
	float cfs[13];
	long nlorder=0;
	(PHR4000_GetCoefficients_LVD)("HR4C2050",cfs,&nlorder);
