...
function pushbutton15_Callback(hObject, eventdata, handles)
    ...
    if(strcmp(class(tt),'ftp')==0)
       ...
       % ascii(tt);
        ...
        mpath=strcat(get(handles.edit18,'String'),datestr(now,'yymmdd'),'\');
    ...
    while(mstop==0)
        ...
        for(i=1:size(ftpdirlist,1))
            if(strcmp(ftpdirlist(i).name,'espec_prepare'))
                ...
                %in aa first line - run number, second - shot number
                set(handles.cshot,'String',aa(2));
                
                %first - check whether the run dir exists. if not - create
                pathWithRun=sprintf('%s%s%03d%s',mpath,'Run',aa(1),'\');
                if(exist(pathWithRun,'dir')==0)
                    mkdir(pathWithRun);
                end
                
                %construct file names for lanes, log and spectrum:
                lanex_file=sprintf('%s%s%04d%s',pathWithRun,'shot',aa(2),'_lanex.png');
                spec_file=sprintf('%s%s%04d%s',pathWithRun,'shot',aa(2),'_spec.txt');
                ...
                [qp_en qp_ch]=dofile(handles,lanex_file,spec_file,log_file,aa(2));
                
                %save results at the server
                mkdir(tt,sprintf('%s%03d','Run',aa(1)));
                cd(tt,sprintf('%s%03d','Run',aa(1)));
                ...
                fprintf(fid,'%f\r\n',qp_ch);
                %paths for files:
                fprintf(fid,'%s%03d%s','Run',aa(1),lanex_file);
                if(get(handles.checkbox3,'Value')==1)
                    fprintf(fid,'%s%03d%s','Run',aa(1),spec_file);
                end
                if(get(handles.checkbox2,'Value')==1)
                    fprintf(fid,'%s%03d%s','Run',aa(1),log_file); 
                end
                fclose(fid);
