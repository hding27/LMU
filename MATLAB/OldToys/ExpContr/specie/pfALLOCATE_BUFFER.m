...
[error_code,ret_bufev] = calllib('PCO_PF_SDK', 'SETBUFFER_EVENT',board_handle,ret_bufnr,ev_ptr);
 if(error_code~=0)
  disp(['error in SETBUFFER_EVENT for bufnr ',int2str(bufnr),' error is ',int2str(error_code)]);    
 end 
%end
