function varargout = raphaella_v1507(varargin)

    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @raphaella_v1507_OpeningFcn, ...
                   'gui_OutputFcn',  @raphaella_v1507_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before raphaella_v1507 is made visible.
function raphaella_v1507_OpeningFcn(hObject, eventdata, handles, varargin)
...
%    fprintf(mf,'cam_id:\t%d\r\n',str2num(get(handles.edit24,'String')));    
        mcell=textscan(fgetl(mf),'%s\t%s');                    
        set(handles.edit24,'String',mcell{2});
 
%fprintf(mf,'ftp_path:\t%s\r\n',get(handles.edit17,'String'));
        mcell=textscan(fgetl(mf),'%s\t%s');                    
        set(handles.edit17,'String',mcell{2});

%fprintf(mf,'loc_path:\t%s\r\n',get(handles.edit18,'String'));
        mcell=textscan(fgetl(mf),'%s\t%s');                    
        set(handles.edit18,'String',mcell{2});

%fprintf(mf,'udp_port:\t%s\r\n',get(handles.edit22,'String'));
        mcell=textscan(fgetl(mf),'%s\t%s');                    
        set(handles.edit22,'String',mcell{2});

%fprintf(mf,'file_tag:\t%s\r\n',get(handles.edit23,'String'));        
        mcell=textscan(fgetl(mf),'%s\t%s');                    
        set(handles.edit23,'String',mcell{2});
...
%    fprintf(mf,'tri_r:\t%g\r\n',str2double(get(handles.tritium_right,'String'))); 
    mv=get(handles.edit24,'String');
    fprintf(mf,'cam_id:\t%s\r\n',mv{1});    
    mv=get(handles.edit17,'String');
    fprintf(mf,'ftp_path:\t%s\r\n',mv{1});
    mv=get(handles.edit18,'String');
    fprintf(mf,'loc_path:\t%s\r\n',mv{1});
    mv=get(handles.edit22,'String');
    fprintf(mf,'udp_port:\t%s\r\n',mv{1});
    mv=get(handles.edit23,'String');
    fprintf(mf,'file_tag:\t%s\r\n',mv{1});
    fclose(mf);
   

% --- Outputs from this function are returned to the command line.
function varargout = raphaella_v1507_OutputFcn(hObject, eventdata, handles)
...
function openShot(handles, mfile, mpath)
    ...
    dofile(handles,strcat(mpath,mfile),sprintf('%sshot%s_spec.txt',mpath,cc),strcat(mpath,'log.txt'), str2num(cc));
...
function pushbutton2_Callback(hObject, eventdata, handles)
    ...
    while(mstop==0)
           %shao-wei
        if ((exist(mnextfile,'file')==0)&& (get(handles.checkbox8,'Value')==0))
...
%    tri_off=str2double(get(handles.edit8,'String'));
    ...
    fixScale=str2double(get(handles.edit19,'String'))/16;
...
%     end    
    colormap(jet(256));
    imagesc(imim2,[0 fixScale]);
    %set(gca,'CLim',[0 fixScale]);
    axes(handles.axes1);
    bg_val=0;
    ...
    if((xl~=0)&&(xr~=0)&&(yt~=0)&&(yb~=0))
        ...
        %%
        %imwrite(imim(yt:yb,xl:xr),strcat(lanex_file(1:length(lanex_file)-4),'_roi.png'),'png','bitdepth',8);
        %%
...
function pushbutton13_Callback(hObject, eventdata, handles)
    ...
    drawnow;
    handles.exposure=str2num(get(handles.edit21,'String'));
...
function openCam(hObject, eventdata,handles)
    ...
    handles.exposure = 10; % exposure time in ms
    handles.gain = 0;
    ...
    handles.color=0;
    %board_number=0;
    ibuff=get(handles.edit24,'String');
    board_number=str2num(ibuff{1});
...
function mimage=grabSingle(hObject, eventdata, handles, swtriggered)
    ...
    if(mStop==0)
        ...
        if(error_code==0)
            clear mimage;
            [error_code,mimg]= pfCOPY_BUFFER(handles.bufaddress,handles.bit_pix,handles.image_width,handles.image_height);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    global mstop;
    mstop=0;
    useftp=get(handles.checkbox6,'Value');
    swtrig=get(handles.checkbox10,'Value');
    ...
    handles.exposure=300;
    maddr=get(handles.edit17,'String');
    maddr=maddr{1};
    mind=find(maddr=='/',1,'first');
    if(isempty(mind))
        maddr_ip=maddr;
        maddr_rp='/';
    else
        maddr_ip=maddr(1:find(maddr=='/',1,'first')-1);
        maddr_rp=maddr(find(maddr=='/',1,'first'):end);
    end
    ...
    ibuff=get(handles.edit22,'String');
    mudp=pnet('udpsocket',str2num(ibuff{1}));
    pnet(mudp,'setreadtimeout',0.05);
...
%    end
    ...
    while(mstop==0)
        ...
        len=pnet(mudp,'readpacket');
        if len==17
	
                mdata=pnet(mudp,'read',17,'char');        
                tic
                pause(0.100);
                
                
                %in mdata XXXXX_XXXXX_XXXXX => run_shot_delay
                mrun=mdata(1:5);
                mshot=str2num(mdata(7:11));
                set(handles.edit25,'String',mdata);
                
                %first - check whether the globaldir exists. if not - create:
                mpath=get(handles.edit18,'String');
                mpath=mpath{1};
                mtag=get(handles.edit23,'String');
                mtag=mtag{1};
                if(exist(mpath,'dir')==0)
                    mkdir(mpath);
                end
                ...
                %second - check whether the run dir exists. if not - create
                pathWithRun=strcat(mpath,mrun,'\');
                if(exist(pathWithRun,'dir')==0)
                    mkdir(pathWithRun);
                end
                set(handles.edit1,'String',pathWithRun);
                %construct file names for lanex, log and spectrum:
                lanex_file=strcat(pathWithRun,mdata,'_',mtag,'.png');
                spec_file=strcat(pathWithRun,mdata,'_',mtag,'_spec.txt');
                log_file=strcat(pathWithRun,mtag,'_log.txt');
                
                %now - acquire image!
                mimage=uint16(grabSingle(hObject, eventdata, handles,swtrig));
                if(isempty(find(mimage~=1,1))==1)
                    axes(handles.axes1);
                    image(mimage);
                    text(size(mimage,1)/2,size(mimage,2)/2,'no trigger?!?!','FontSize',18,'Color',[rand rand rand]);
                    ...
                    clear mimage;
                    %process spectrum picture:
                    [qp_en qp_ch]=dofile(handles,lanex_file,spec_file,log_file,mshot);
